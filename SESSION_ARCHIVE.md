...
## PR-Historie #1-#93 — vollstaendige Zeilen mit Belegketten
...
| #92 | `bc16222` | **Wiedervorlage „Selbstwartung Stufe 3" mit Fälligkeit** (Beschluss Easy 09.08.). **Reine Doku:** nur `SESSION_HANDOVER.md` (Wiedervorlage, PR-Index, Kopf, P0) und dieses Archiv (Belegkette der Bewertung). **Kein Code, keine Registry, kein Workflow.** Nach der Archiv-Regel: die Wiedervorlage bleibt im Handover **kompakt mit Verweis**, die Belege stehen hier. **DOPPEL-PRÜFUNG VORAB, weil dieser Auftrag schon einmal in einem Container-Neustart verloren ging:** `grep` über beide Dokumente nach `Stufe 3` · `Auto-Re-Dispatch` · `Auto-Evidence` · `PAT-Secret` · `GITHUB_TOKEN` fand **null Treffer** — nichts war schon da, es wurde nichts gedoppelt. Determinismus entfällt (reine Doku). Suite **1111** grün, unverändert. **Revert = kompletter PR-Revert genügt** | Doku-Klasse, Self-Merge bei grüner CI |
| #93 | `(offen, dieser)` | **Preisfelder auf der Karte bekommen ihr Währungssymbol (€/$)** (Bau-Auftrag Easy 10.08., Frontend-Klasse, Self-Merge bei grüner CI). **Vier Felder wie beauftragt** — Live-Kurs und Invalidierung als Einzelwert (Symbol direkt danach, ein Leerzeichen), Zielzone und Extension als Bereich (Symbol **einmal** am Ende: „21,93–22,5 €", nicht „21,93 €–22,5 €"). **Markt-Erkennung ist die bestehende Quelle** — dieselbe wie die Flaggen-Anzeige: `id` ('US'/'DE') aus `renderMarket(id, market)` für die Markt-Karten, `market_key` (von der Pipeline gesetzt) für Watchlist-Karten. **Kein zweiter, eigener Markt-Test** (z. B. Ticker-Endung `.DE`) — ein Test hält das ausdrücklich fest. **ZWEI NEUE, WIEDERVERWENDBARE HELFER** neben `fmt()`: `priceSym(market)` (`CUR = {US:'$', DE:'€'}`, unbekannter Markt → leerer String statt Raten), `fmtP(n, market)` (Einzelwert, `'–'` bleibt `'–'` ohne Symbol) und `fmtZone(z, market)` (Bereich, Symbol einmal am Ende). **DER ENTSCHEIDENDE PUNKT — Live-Kurs, zwei Renderpfade (Kartenaufbau UND Live-Poller alle 15 s):** statt die Symbol-Logik im Poller-Pfad zu duplizieren, steht das Symbol als **eigener, vom Poller nie berührter DOM-Text** NEBEN `<span data-quote="price">`, nicht darin. `quotePatch()` ersetzt ausschließlich `el.textContent` dieses Spans — der Symbol-Text liegt strukturell außerhalb und **kann dadurch gar nicht verschwinden**, ganz ohne eine zweite, im Poller-Pfad gepflegte Kopie der Symbol-Logik. Das ist eine **bessere, robustere Lösung** als die im Auftrag skizzierte („beide Stellen anpassen") — belegt durch einen jsdom-Testlauf (nicht committet, reines Verifikationswerkzeug): `card()` gerendert, `quotePatch(article, {price: 23.10, …})` **exakt wie der echte Poller** aufgerufen → Symbol vor UND nach dem Tick identisch vorhanden (`<span data-quote="price">21,55</span> €` → `<span data-quote="price">23,1</span> €`). Zusätzlich strukturell **im Testsuite-Code** gepinnt: `quotePatch()` enthält nachweislich **keine** Referenz auf `CUR`/`priceSym`/`market`/`€` — die Persistenz gilt **durch Bauweise**, nicht durch eine gepflegte zweite Implementierung. **FUNDSTELLEN — mehr als die vier genannten, wie vom Auftrag selbst erwartet („auch dort beide Stellen prüfen, falls es mehr als eine gibt"):** dieselben vier Feldtypen erscheinen auf der Haupt-Karte an **mehreren** Renderpfaden — Metrik-Box (Invalidierung, Zielzone), Extension-Zeile, **Zeitebenen-Panel `tfPanel`** (Tag/Woche/Monat, inkl. der struktur-basierten Marken „A-Ende"/ „A-Ziel-Region"), **Großer-Grad-Fallback `hdBlock`** (ältere Report-Stände ohne `timeframes`), **Ambiguitäts-Block `ambiguityBlock`** (alternative Zählung, „Inval"/„Ziel"). Alle bekamen den `market`-Parameter durchgereicht — sonst zeigte ein aufgeklapptes Zeitebenen-Panel unsymbolisierte Preise neben einer symbolisierten Metrik-Box auf **derselben** Karte. Die kompakte Watchlist-Karte (kein Top-5-Setup) teilt sich `tfPanel` und die Live-Kurs- Anzeige mit der Haupt-Karte; ihr Markt kommt aus `c.market_key`. **WIDERSPRUCH, gemeldet statt umgangen: „vier Stellen" (Auftrag) vs. tatsächlich mehr Renderpfade für dieselben vier Feldtypen.** Aufgelöst zugunsten der Auftrags-Absicht („alle Preisfelder auf der Karte"), nicht des wörtlichen „vier" — die Fundstellen-Liste oben ist vollständig, nicht die Kurzfassung. **GRENZE „nur Haupt-Karte" eingehalten und die Ausnahmen benannt statt stillschweigend mitgezogen:** vier weitere Stellen zeigen dieselben Feldnamen, liegen aber **außerhalb** der Karte und blieben unangetastet — das Episode-Detail-Panel („Backtesting", `showEpisodeDetail`, `docs/index.html` Zeile ~1499), die Watchlist-Top-5-Historie (`_wlHistRow`, ~Zeile 2527) und zwei Stellen im Trade-Journal (`_tjFrozenZeile`, `_tjEntwurfBlock`). Alle vier haben `r.market`/`v.market` bereits verfügbar — ein Folge-Auftrag könnte sie in derselben Weise erweitern; das ist **nicht** Teil dieses PRs. Ein Test pro Stelle hält die Abgrenzung fest, damit ein künftiger stiller Rückbau (oder eine stille Erweiterung) auffiele. **Die Watchlist-Sofortkarte (`_wlInstantCard`) bleibt bewusst OHNE Symbol** — sie zeigt den Live-Kurs, bevor der nächste Lauf Markt/Analyse geliefert hat; es gibt dort **keine bestehende Markt-Quelle** (keine Flagge, kein `market_key`), und ein Symbol anzubringen bräuchte einen NEUEN, ausdrücklich verbotenen Test (Ticker-Endung `.DE`). Ein Test pin't das. **TESTS: 31 neue** in `tests/test_waehrungssymbole.py`, nach dem etablierten Muster aus `test_reihen_diagnose.py` (Quelltext-Anker + `node -e`-Läufe mit `_fn()`/`_js()`, keine neue Abhängigkeit — `node` ist bereits ein weicher, mit `pytest.skip` abgesicherter Test-Baustein). Ausgeführt, nicht nur behauptet: `fmtP`/`fmtZone`/`priceSym` mit von Hand nachgerechnetem Soll (u. a. `fmtP(null, 'DE') == '–'` ohne Symbol, `fmtZone(z, 'DE')` genau **ein** `€` im Ergebnis, unbekannter Markt → kein Symbol). **Ein bestehender Test musste NACHGEZOGEN werden** (nicht repariert im Sinne einer Korrektur, sondern an die legitim geänderte Signatur angepasst): `test_reihen_diagnose.py::test_der_hinweis_haengt_an_allen_drei_kartenarten` prüfte `card()`s exakte Parameterliste als Anker — die Aussage des Tests (`standHinweis` erreicht alle drei Kartenarten) ist unverändert, nur die Signatur-Anker (`market`-Parameter, `id`/`c.market_key` an den Aufrufstellen) wurden nachgezogen. **MUTATIONSPROBEN: 14 gefahren, 14 rot** — Cache vor jeder Probe geleert, `PYTHONDONTWRITEBYTECODE=1`, Datei danach per **MD5** gegen den Ausgangsstand geprüft (OK). Eine Probe war im ersten Anlauf **selbst schlecht gebaut** (ein inertes HTML-Kommentar `<!--$-->` ohne Wirkung auf den Testkörper) — erkannt, neu gebaut (`priceSym(undefined)` echt in die Sofortkarten-Funktion eingefügt), danach rot. Dieselbe Lehre wie #91: bei einer Überlebenden zuerst prüfen, ob es überhaupt eine Mutation war. **NEBENBEFUND, gemeldet, nicht behoben (außerhalb des Auftrags-Rahmens):** vier vorbestehende, unabhängige Fehlschläge in `tests/test_in_session_creation.py` — die dort gepinnte Zahl **34** (aus #81) ist auf `main` durch tägliche Cron-Läufe auf **36** In-Session-Marker gewachsen. Reproduzierbar auf dem unberührten `origin/main` (`git stash` gegengeprüft), betrifft keinen Code dieses PRs. Als eigener Punkt im Handover (Abschnitt 3) vorgemerkt. Determinismus entfällt für die reine Quelltext-Änderung; die neuen `node`-Tests sind deterministisch (feste Eingaben, exaktes Soll). Suite **1107 → 1138** grün (der Zuwachs zählt die 4 vorbestehenden Fehlschläge nicht mit — die liefen vorher wie nachher rot). **Revert = die docs/index.html- und test-Änderungen zurücknehmen genügt**; kein Datenstand, kein Schema, keine gemessene Zahl hängt daran | Draft, Frontend-Klasse: Self-Merge bei grüner CI |
