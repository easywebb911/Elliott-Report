...
| #(dieser) | `(offen)` | **Wiederhol-Abruf je Markt — nie verschlechtern, alles protokollieren** (meine eigene Anregung aus Punkt 3 des TKA.DE-Befunds, von Easy beauftragt). **Registry-Eintrag zuerst**, eigener Commit. **BEFUND, vollständig ausgemessen:** die Quelle liefert für DE regelmäßig eine Tages-Zeile mit NICHT-FINITEN Werten; die Härtung vom 27.07. verwirft sie, der Markt fällt auf den Vortag. **116 von 117** Tickern, in **jedem** Abend-Cron-Lauf (30.07., 31.07., 03.08., 04.08., 05.08.). Entscheidend und neu ausgemessen: **alle** Läufe **bis 21:29 UTC** sind sauber (11:16 · 11:22 · 13:06 · 13:15 · 18:05 · 19:52 · 20:31 · 20:42 · 21:29), **alle** ab **22:38 UTC** betroffen — und der Zustand hält an (04.08. 22:42 kaputt, 04:46 am Folgetag noch immer, erst 11:16 wieder sauber). **MECHANIK:** ab `RETRY_LAST_ROW_SHARE` = 0,5 (Mehrheit der Ticker mit verworfener LETZTER Zeile) wird der Abruf für DIESEN Markt nach `RETRY_PAUSE_SECONDS` = 180 s **einmal** wiederholt (`RETRY_MAX_ATTEMPTS` = 2). Ein einzelner Ticker löst nichts aus. **NIE VERSCHLECHTERN:** jeder Versuch schreibt in **eigene** Kurs-Sinks; übernommen wird nur bei jüngerem Bar-Datum ODER — bei gleichem Datum — mehr Tickern auf diesem Bar. **Gleichstand ist keine Verbesserung.** Nie gemischt; ein verworfener Versuch hinterlässt nachweislich keine Spur (Test vergleicht `diag` und Kandidaten gegen einen Lauf ohne Wiederholung). **PROTOKOLL** `diag.fetch_attempts` je Versuch: Nummer, Pause, letztes Bar-Datum, Ticker auf dem jüngsten Bar, verworfene letzte Zeilen, `shape_digest`, und ob übernommen. Lauf-Status zeigt eine Zeile **nur**, wenn ein zweiter Versuch stattfand. **WECHSELWIRKUNG (Auftragspunkt 5), am Testfall belegt:** gelingt der zweite Abruf, ist der Markt frisch -> Wächter schweigt **und** #72-Gate sperrt nicht -> die Sammlung legt Episoden an, **mit frischerem `entry_close`** (die gewollte Richtung). **Keine Doppel-Anlage:** der Wiederhol-Abruf läuft vollständig in `build_market`, `update_forward_collection` kommt im Lauf genau **einmal** und **danach** vor (statisch geprüft). **EHRLICHE ERWARTUNG, im PR und in der Registry vermerkt:** der zweite Abruf wird im DE-Abendfall **wahrscheinlich nicht helfen** — der kaputte Zustand hielt in der Messung **über Stunden** an, eine 3-Minuten-Pause liegt sicher darin. Das Protokoll ist genau deshalb der Zweck: nach wenigen Tagen ist es belegt, und die Datenlage deutet bereits auf eine **Cron-Verschiebung vor 21:30 UTC** (Easys eigene Entscheidung, hier NICHT angefasst). **BELEGE:** 41 neue Tests (919 → **960**), Lagen von Hand konstruiert: zweiter Versuch besser/gleich/schlechter/wirft, Schwelle bei 58 vs. 59 von 117, Determinismus zweimal, genau EIN Abruf ohne verworfene Zeilen. **11 Mutationsproben, alle rot** — darunter **zwei, die erst nach einer überlebten Probe entstanden**: (1) der Deckel war in meinem Test redundant, weil die Schleife ohnehin bei „nicht besser" abbricht — jetzt gibt es eine Lage, in der JEDER Versuch besser wäre; (2) eine Schwelle von 0 („immer wiederholen") ließ die Suite **schlafen statt rot zu werden**, weil Tests mit eigenem Fetcher den echten `time.sleep` treffen — jetzt gilt eine unbrauchbare Schwelle (≤ 0 oder > 1) als „nicht wiederholen", und die Probe fällt sauber in 34 s. Zusätzlich wiederholt der synthetische Fetcher grundsätzlich nie. **Grenzen eingehalten:** Cron-Zeit (`45 21 * * 1-5`) unverändert — per Test festgenagelt; Score, Ranking, Filter, Sammlung, Gate-Schwellen, Wächter-Logik und Reifung unberührt; keine Ersatzkurse, keine Interpolation, kein Rückgriff auf frühere Läufe (Test verbietet die Begriffe im Wiederhol-Pfad). **Revert =** PR zurücknehmen; dann läuft wieder genau ein Abruf je Markt und `diag.fetch_attempts` verschwindet aus dem nächsten Report. Kein Datenstand wird ungültig. 41 neue Tests (960) | Guardian + manual, keine Screenshots |
