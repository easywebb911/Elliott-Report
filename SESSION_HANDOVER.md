...
allein mit diesem Dokument (plus Repo) weiterarbeiten können. Stand: **01.08.2026**,
nach PR #67 (**Dispatch-Kollision im Tageslauf**, gemerged `c572a4c`); dieser PR:
**Episoden-Anschluss zählt Kalendertage statt Läufe** (Sammlungs-Logik +
additives Schema-Feld), offen. Das **EXZELLENZ-AUDIT P1–P4** ist mit #47 komplett.
Alle Zahlen/Hashes sind gegen `git log` und den Code geprüft, nicht aus dem
Gedächtnis.

> **BRANCH-BASIS:** frisch von `main` (`15aec42` = Datenstand nach dem #67-Merge) abgezweigt.
...
- **OFFEN (Push-Text, eigener Mini-PR) — Score im Alarm ganzzahlig gerundet:** `score_alert_body` schreibt `{score:.0f}` neben `{threshold:.1f}`. Bei einem Score in [88,20; 88,49] stünde „**88** · Ende W4 (Schwelle 88.2)" — der Text läse sich, als läge der Wert UNTER seiner eigenen Schwelle, obwohl der Alarm zu Recht auslöste. Am 31.07. folgenlos (88,69 → „89"). Das Format ist in `tests/test_notify.py` wörtlich gepinnt, also bewusst gesetzt; Auflösung wäre `{score:.1f}`. Von Easy als **eigener Mini-PR** zurückgestellt.
- **BEFUND 01.08. (rein lesend, nichts geändert) — `config` fehlt in `notify.py`, wenn es SELBST das Skript ist:** `config.py` liegt im **Repo-Root**, aber `notify.py` legt nur `REPO_ROOT/"scripts"` auf `sys.path` (Zeile 38). Beim Aufruf `python scripts/notify.py …` ist `sys.path[0]` das **Skript-Verzeichnis**, nicht das Arbeitsverzeichnis — der Repo-Root steht also **nirgends** auf dem Pfad, `import config` scheitert mit `ModuleNotFoundError`, und weil `forward_collection` seinerseits `import config` macht, scheitert auch der. Beide sind in `try/except` gekapselt → `config = None`, `_fc = None`, **still**. Nachgestellt: mit `sys.path[0] = scripts/` scheitern `config` und `forward_collection`, `market_calendar` (liegt in `scripts/`) gelingt. **Warum es „lokal geht":** jede Reproduktion über `python -c` hat `''` (das Arbeitsverzeichnis) auf dem Pfad — die Bedingung ist dort nie erfüllt. **Warum die Pipeline es nicht merkt:** `elliott_pipeline.py` legt **beide** Verzeichnisse auf den Pfad (Zeilen 34–36) und importiert `notify` als Modul; der Score-Alarm-Pfad ist deshalb unbeeinträchtigt. **Konkrete Folge:** `_evaluable_count` liefert dauerhaft 0 → der **Meilenstein-Push bei n≥100 kann nie feuern** — dieselbe Klasse wie der tote Score-Alarm vor #66. Heute folgenlos (n = 0/100). Betroffen sind die drei Skript-Einstiege `--mode daily`, `--mode selftest`, `--mode staleness`. **Fix-Vorschlag (nicht gebaut):** in `notify.py` zusätzlich `sys.path.insert(0, str(REPO_ROOT))`, wie die Pipeline es tut, plus ein Test, der `notify.py` als **Skript** startet und `_fc is not None` prüft. **Easys Entscheidung.**
...
- **OFFEN (erster Lauf nach diesem PR) — Episoden-Anschluss live:** prüfen, dass der erste Lauf `prev_distinct_run_date` setzt und dass ein zweiter Lauf desselben Kalendertags **keinen** neuen Record anlegt. Offline am echten Stand gegengerechnet (alte Regel 50→53 Records, neue 50→50), live noch nicht gesehen.
