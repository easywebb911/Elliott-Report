...
nach PR #77 (**Wegwerf-Mess-Workflow für die Quellen-Zeiten**, gemerged `df68c9d`).
**Zwei PRs stehen offen und warten auf Easy:** **#78** (Umgebungsleck im Test — die
CI auf `main` war dauerrot) und **#79** (dieser: Ready-Meldungs-Regel + dieses
Handover-Update).
Das **EXZELLENZ-AUDIT P1–P4** ist mit #47 komplett.
Alle Zahlen/Hashes sind gegen `git log` und den Code geprüft, nicht aus dem
Gedächtnis.

> **BRANCH-BASIS:** `claude/ready-meldung-regel`, rebast auf `main` (`df68c9d`).
...
- **OFFEN bis #78 gemerged ist — die CI auf `main` ist dauerrot.** Im Fenster
  ...
  **Umgebungsleck im Test**, kein Produktionsfehler (`GITHUB_REF` →
  ...
  #78-Zeile in Abschnitt 2). **Solange #78 nicht gemerged ist, ist „CI rot auf
  `main`" ein wertloses Signal** — es war dauerrot. **Nach dem Merge muss der
  erste Push-Lauf auf `main` grün sein; das ist die Live-Verifikation und sie
  steht noch aus.**
...
1. **#78 mergen** (Umgebungsleck). Solange er offen ist, ist die CI auf `main`
   dauerrot und das Signal wertlos. **Danach den ersten Push-Lauf auf `main`
   ansehen — er muss grün sein.** Das ist die einzige noch offene
   Live-Verifikation dieses PRs.
2. **#79 mergen** (Ready-Meldungs-Regel + dieses Handover-Update).
...
**Prüfe nach dem Merge von #78 den ersten Push-Lauf auf `main`** — das ist die
Live-Verifikation und sie stand bei Redaktionsschluss noch aus.
